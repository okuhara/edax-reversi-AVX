...
int board_score_1(unsigned long long P, int alpha, int pos)
{
	...
	switch (pos) {
		...
		case 56: // A8 (4%)
			...
			if (n_flips == 0) {
				...
				if (score > alpha) {	// lazy cut-off
					if ((n_flips = count_last_flip_A8(~P)) != 0)
		...
		case  8: // A2 (4%)
			...
			// fall through
		case 48: // A7 (6%)
			n_flips = count_last_flip_A8(P << 8);
			score += n_flips;
			...
			if (n_flips == 0) {
				...
				if (score <= 0)
					score = score2;
				if (score > alpha) {	// lazy cut-off
					if ((n_flips = count_last_flip_A8(~P << 8)) != 0)
						score = score2 - n_flips;
				}
			}
			break;
		...
		case 57: // B8 (9%)
			...
			if (n_flips == 0) {
				...
				if (score <= 0)
					score = score2;
				if (score > alpha) {	// lazy cut-off
					if ((n_flips = count_last_flip_B8(~P)) != 0)
						score = score2 - n_flips;
				}
			}
			break;
		...
		case  9: // B2 (2%)
			...
			// fall through
		case 49: // B7 (5%)
			n_flips = count_last_flip_B8(P << 8);
			score += n_flips;
			...
			if (n_flips == 0) {
				...
				if (score <= 0)
					score = score2;
				if (score > alpha) {	// lazy cut-off
					if ((n_flips = count_last_flip_B8(~P << 8)) != 0)
						score = score2 - n_flips;
				}
			}
			break;
		...
		case  2: // C1 (0.2%)
			...
			// fall through
		case 58: // C8 (1%)
			n_flips  = count_V_flip_reverse((P & 0x0404040404040404), 13);
			n_flips += COUNT_FLIP_2[P >> 56];
			n_flips += COUNT_FLIP_2[t1 = ((P & 0x040A112040800000) * 0x0101010101010101) >> 56];	// A6C8H3
			score += n_flips;
		...
		case  3: // D1 (0.4%)
			...
			// fall through
		case 59: // D8 (2%)
			n_flips  = count_V_flip_reverse((P & 0x0808080808080808), 12);
			n_flips += COUNT_FLIP_3[P >> 56];
			n_flips += COUNT_FLIP_3[t1 = ((P & 0x0814224180000000) * 0x0101010101010101) >> 56];	// A5D8H4
			score += n_flips;
		...
		case  4: // E1 (0.7%)
			...
			// fall through
		case 60: // E8 (2%)
			n_flips  = count_V_flip_reverse((P & 0x1010101010101010), 11);
			n_flips += COUNT_FLIP_4[P >> 56];
			n_flips += COUNT_FLIP_4[t1 = ((P & 0x1028448201000000) * 0x0101010101010101) >> 56];	// A4E8H5
			score += n_flips;
		...
		case  5: // F1 (0.3%)
			...
			// fall through
		case 61: // F8 (2%)
			n_flips  = count_V_flip_reverse((P & 0x2020202020202020), 10);
			n_flips += COUNT_FLIP_5[P >> 56];
			n_flips += COUNT_FLIP_5[t1 = ((P & 0x0050880402010000) * 0x0101010101010101) >> 56];	// A3F8H6
			score += n_flips;
		...
		case 62: // G8 (8%)
			...
			if (n_flips == 0) {
				...
				if (score <= 0)
					score = score2;
				if (score > alpha) {	// lazy cut-off
					if ((n_flips = count_last_flip_G8(~P)) != 0)
						score = score2 - n_flips;
				}
			}
			break;
		...
		case 14: // G2 (4%)
			...
			// fall through
		case 54: // G7 (5%)
			n_flips = count_last_flip_G8(P << 8);
			score += n_flips;
			...
			if (n_flips == 0) {
				...
				if (score <= 0)
					score = score2;
				if (score > alpha) {	// lazy cut-off
					if ((n_flips = count_last_flip_G8(~P << 8)) != 0)
						score = score2 - n_flips;
				}
			}
			break;
		...
		case 63: // H8 (4%)
			...
			if (n_flips == 0) {
				...
				if (score <= 0)
					score = score2;
				if (score > alpha) {	// lazy cut-off
					if ((n_flips = count_last_flip_H8(~P)) != 0)
						score = score2 - n_flips;
				}
			}
			break;
		...
		case 15: // H2 (6%)
			...
			// fall through
		case 55: // H7 (8%)
			n_flips = count_last_flip_H8(P << 8);
			score += n_flips;
			...
			if (n_flips == 0) {
				...
				if (score <= 0)
					score = score2;
				if (score > alpha) {	// lazy cut-off
					if ((n_flips = count_last_flip_H8(~P << 8)) != 0)
						score = score2 - n_flips;
				}
			}
			break;
		...
		case  0: // A1
			...
			if (n_flips == 0) {	// (23%)
				...
				if (score > alpha) {	// lazy cut-off (40%)
					if ((n_flips = count_last_flip_A1(~P)) != 0)	// (98%)
						score = score2 - n_flips;
				}
			}
			break;
		...
		case 48: // A7
			...
			// fall through
		case  8: // A2
			n_flips = count_last_flip_A1(P >> 8);
			score += n_flips;
			...
			if (n_flips == 0) {
				...
				if (score <= 0)
					score = score2;
				if (score > alpha) {	// lazy cut-off
					if ((n_flips = count_last_flip_A1(~P >> 8)) != 0)
						score = score2 - n_flips;
				}
			}
			break;
		...
		case  1: // B1
			...
			if (n_flips == 0) {
				...
				if (score <= 0)
					score = score2;
				if (score > alpha) {	// lazy cut-off
					if ((n_flips = count_last_flip_B1(~P)) != 0)
						score = score2 - n_flips;
				}
			}
			break;
		...
		case 49: // B7
			...
			// fall through
		case  9: // B2
			n_flips = count_last_flip_B1(P >> 8);
			score += n_flips;
			...
			if (n_flips == 0) {
				...
				if (score <= 0)
					score = score2;
				if (score > alpha) {	// lazy cut-off
					if ((n_flips = count_last_flip_B1(~P >> 8)) != 0)
						score = score2 - n_flips;
				}
			}
			break;
		...
		case  2: // C1
			P_v = P & 0x0404040404040400;
			n_flips  = ((P_v & -P_v) * 0x0000081018202830) >> 60;
			n_flips += COUNT_FLIP_2[P & 0xff];
			n_flips += COUNT_FLIP_2[t1 = ((P & 0x0000804020110A04) * 0x0101010101010101) >> 56];	// A3C1H6
			score += n_flips;
		...
		case  3: // D1
			P_v = P & 0x0808080808080800;
			n_flips  = ((P_v & -P_v) * 0x000004080c101418) >> 60;
			n_flips += COUNT_FLIP_3[P & 0xff];
			n_flips += COUNT_FLIP_3[t1 = ((P & 0x0000008041221408) * 0x0101010101010101) >> 56];	// A3C1H6
			score += n_flips;
		...
		case  4: // E1
			P_v = P & 0x1010101010101000;
			n_flips  = ((P_v & -P_v) * 0x0000020406080a0c) >> 60;
			n_flips += COUNT_FLIP_4[P & 0xff];
			n_flips += COUNT_FLIP_4[t1 = ((P & 0x0000000182442810) * 0x0101010101010101) >> 56];	// A5E1H4
			score += n_flips;
		...
		case  5: // F1
			P_v = P & 0x2020202020202000;
			n_flips  = ((P_v & -P_v) * 0x0000010203040506) >> 60;
			n_flips += COUNT_FLIP_5[P & 0xff];
			n_flips += COUNT_FLIP_5[t1 = ((P & 0x0000010204885020) * 0x0101010101010101) >> 56];	// A6F1H3
			score += n_flips;
		...
		case  6: // G1
			...
			if (n_flips == 0) {
				...
				if (score <= 0)
					score = score2;
				if (score > alpha) {	// lazy cut-off
					if ((n_flips = count_last_flip_G1(~P)) != 0)
						score = score2 - n_flips;
				}
			}
			break;
		...
		case 54: // G7
			...
			// fall through
		case 14: // G2
			n_flips = count_last_flip_G1(P >> 8);
			score += n_flips;
			...
			if (n_flips == 0) {	// (23%)
				...
				if (score <= 0)
					score = score2;
				if (score > alpha) {	// lazy cut-off (40%)
					if ((n_flips = count_last_flip_G1(~P >> 8)) != 0)
						score = score2 - n_flips;
				}
			}
			break;
		...
		case  7: // H1
			...
			if (n_flips == 0) {
				...
				if (score <= 0)
					score = score2;
				if (score > alpha) {	// lazy cut-off
					if ((n_flips = count_last_flip_H1(~P)) != 0)
						score = score2 - n_flips;
				}
			}
			break;
		...
		case 55: // H7
			...
			// fall through
		case 15: // H2
			n_flips = count_last_flip_H1(P >> 8);
			score += n_flips;
			...
			if (n_flips == 0) {
				...
				if (score <= 0)
					score = score2;
				if (score > alpha) {	// lazy cut-off
					if ((n_flips = count_last_flip_H1(~P >> 8)) != 0)
						score = score2 - n_flips;
				}
			}
			break;
		...
		case 32: // A5 (0.5%)
			...
			// fall through
		case 24: // A4 (0.2%)
			n_flips  = COUNT_FLIP_3[t1 = ((P & 0x1008040201010101) * 0x0102040808080808) >> 56];	// A1A4E8
			n_flips += count_H_flip_right(P, 24);
			n_flips += COUNT_FLIP_4[t2 = ((P & 0x0101010101020408) * 0x1010101008040201) >> 56];	// D1A4A8
			score += n_flips;
		...
		case 39: // H5 (1%)
			...
			// fall through
		case 31: // H4 (0.8%)
			n_flips  = COUNT_FLIP_3[t1 = ((P & 0x8080808080402010) * 0x0010101010204081) >> 56];	// E1H4H8
			n_flips += count_H_flip_left(P, 31, 0x7f);
			n_flips += COUNT_FLIP_4[t2 = (((P & 0x0810204080808080) >> 3) * 0x0804020101010101) >> 56];	// H1H4D8
			score += n_flips;
		...
		case 16: // A3 (0.4%)
			...
			// fall through
		case 40: // A6 (0.5%)
			n_flips  = COUNT_FLIP_5[t1 = ((P & 0x0402010101010101) * 0x0102040810202020) >> 56];	// A1A6C8
			n_flips += COUNT_FLIP_2[t2 = ((P & 0x0101010204081020) * 0x0404040404040201) >> 56];	// F1A6A8
			n_flips += count_H_flip_right(P, 40);
			score += n_flips;
		...
		case 23: // H3 (1%)
			...
			// fall through
		case 47: // H6 (1%)
			n_flips  = COUNT_FLIP_5[t1 = ((P & 0x8080804020100804) * 0x0040404040404081) >> 56];	// C1H6H8
			n_flips += COUNT_FLIP_2[t2 = (((P & 0x2040808080808080) >> 5) * 0x2010080402010101) >> 56];	// H1H6F8
			n_flips += count_H_flip_left(P, 47, 0x7f);
			score += n_flips;
			...
			break;
  #else
		case 16: case 23: case 40: case 47:
  #endif
