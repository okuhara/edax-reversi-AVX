...
unsigned long long get_moves_mmx(const unsigned long long P_, const unsigned long long O_)
{
	...
	flip = _m_por(flip, _m_pand(O, _m_psllqi(flip, 8)));     	movesL = mO1 + flip1;
	pre  = _m_pand(O, _m_psllqi(O, 8));
	flip = _m_por(flip, _m_pand(pre, _m_psllqi(flip, 16)));
	flip = _m_por(flip, _m_pand(pre, _m_psllqi(flip, 16)));
	M = _m_psllqi(flip, 8);
	    /* shift = -8 */                                     	    /* shift = -1 */
	flip = _m_pand(O, _m_psrlqi(P, 8));                      	flip1  = mO1 & ((unsigned int) P_ >> 1);
	flip = _m_por(flip, _m_pand(O, _m_psrlqi(flip, 8)));     	flip1 |= mO1 & (flip1 >> 1);
	pre  = _m_psrlqi(pre, 8);                                	pre1   = mO1 & (mO1 >> 1);
	...
	flip = _m_por(flip, _m_pand(mO, _m_psrlqi(flip, 7)));    	movesH = mO1 + flip1;
	pre  = _m_psrlqi(pre, 7);
	flip = _m_por(flip, _m_pand(pre, _m_psrlqi(flip, 14)));
	flip = _m_por(flip, _m_pand(pre, _m_psrlqi(flip, 14)));
	M = _m_por(M, _m_psrlqi(flip, 7));
	    /* shift = +9 */                                     	    /* shift = -1 */
	...
	pre  = _m_pand(mO, _m_psllqi(mO, 9));                    	pre1   = mO1 & (mO1 >> 1);
...
int get_stability_fulls(unsigned long long P, unsigned long long O, unsigned long long full[5])
{
	...
	SL = PL & 0x7e7e7e00;	SH = PH & 0x007e7e7e;
