...
static int count_last_flip_A1(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_0[((P & 0x0101010101010100ULL) * 0x0102040810204080ULL) >> 57] & 0xff;
	n_flipped += COUNT_FLIP_0[(P >> 1) & 0x7f] & 0xff;
	n_flipped += COUNT_FLIP_0[((P & 0x8040201008040200ULL) * 0x0101010101010101ULL) >> 57] & 0xff;
...
static int count_last_flip_B1(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_0[((P & 0x0202020202020200ULL) * 0x0081020408102040ULL) >> 57] & 0xff;
	n_flipped += COUNT_FLIP_1[(P >> 2) & 0x3f] & 0xff;
	n_flipped += COUNT_FLIP_1[((P & 0x0080402010080400ULL) * 0x0101010101010101ULL) >> 58] & 0xff;
...
static int count_last_flip_C1(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_0[((P & 0x0404040404040400ULL) * 0x0040810204081020ULL) >> 57] & 0xff;
	n_flipped += COUNT_FLIP_2[P & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_2[(P & 0x0000804020110a04ULL) * 0x0101010101010101ULL >> 56] & 0xff;	// A3C1H6
...
static int count_last_flip_D1(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_0[((P & 0x0808080808080800ULL) * 0x0020408102040810ULL) >> 57] & 0xff;
	n_flipped += COUNT_FLIP_3[P & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_3[(P & 0x0000008041221408ULL) * 0x0101010101010101ULL >> 56] & 0xff;	// A4D1H5
...
static int count_last_flip_E1(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_0[((P & 0x1010101010101000ULL) * 0x0010204081020408ULL) >> 57] & 0xff;
	n_flipped += COUNT_FLIP_4[P & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_4[(P & 0x0000000182442810ULL) * 0x0101010101010101ULL >> 56] & 0xff;	// A5E1H4
...
static int count_last_flip_F1(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_0[((P & 0x2020202020202000ULL) * 0x0008102040810204ULL) >> 57] & 0xff;
	n_flipped += COUNT_FLIP_5[P & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_5[(P & 0x0000010204885020ULL) * 0x0101010101010101ULL >> 56] & 0xff;	// A6F1H3
...
static int count_last_flip_G1(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_0[((P & 0x4040404040404000ULL) * 0x0004081020408102ULL) >> 57] & 0xff;
	n_flipped += COUNT_FLIP_6[P & 0x3f] & 0xff;
	n_flipped += COUNT_FLIP_6[((P & 0x0001020408102000ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_H1(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_0[((P & 0x8080808080808000ULL) * 0x0002040810204081ULL) >> 57] & 0xff;
	n_flipped += COUNT_FLIP_7[P & 0x7f] & 0xff;
	n_flipped += COUNT_FLIP_7[((P & 0x0102040810204000ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_A2(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_1[((P & 0x0101010101010000ULL) * 0x0102040810204080ULL) >> 58] & 0xff;
	n_flipped += COUNT_FLIP_0[(P >> 9) & 0x7f] & 0xff;
	n_flipped += COUNT_FLIP_1[((P & 0x4020100804020000ULL) * 0x0101010101010101ULL) >> 57] & 0xff;
...
static int count_last_flip_B2(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_1[((P & 0x0202020202020000ULL) * 0x0081020408102040ULL) >> 58] & 0xff;
	n_flipped += COUNT_FLIP_1[(P >> 10) & 0x3f] & 0xff;
	n_flipped += COUNT_FLIP_1[((P & 0x8040201008040000ULL) * 0x0101010101010101ULL) >> 58] & 0xff;
...
static int count_last_flip_C2(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_1[((P & 0x0404040404040000ULL) * 0x0040810204081020ULL) >> 58] & 0xff;
	n_flipped += COUNT_FLIP_2[(P >> 8) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_2[(P & 0x00804020110a0400ULL) * 0x0101010101010101ULL >> 56] & 0xff;	// A4C2H7
...
static int count_last_flip_D2(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_1[((P & 0x0808080808080000ULL) * 0x0020408102040810ULL) >> 58] & 0xff;
	n_flipped += COUNT_FLIP_3[(P >> 8) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_3[(P & 0x0000804122140800ULL) * 0x0101010101010101ULL >> 56] & 0xff;	// A5D2H6
...
static int count_last_flip_E2(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_1[((P & 0x1010101010100000ULL) * 0x0010204081020408ULL) >> 58] & 0xff;
	n_flipped += COUNT_FLIP_4[(P >> 8) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_4[(P & 0x0000018244281000ULL) * 0x0101010101010101ULL >> 56] & 0xff;	// A6E2H5
...
static int count_last_flip_F2(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_1[((P & 0x2020202020200000ULL) * 0x0008102040810204ULL) >> 58] & 0xff;
	n_flipped += COUNT_FLIP_5[(P >> 8) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_5[(P & 0x0001020488502000ULL) * 0x0101010101010101ULL >> 56] & 0xff;	// A7F2H4
...
static int count_last_flip_G2(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_1[((P & 0x4040404040400000ULL) * 0x0004081020408102ULL) >> 58] & 0xff;
	n_flipped += COUNT_FLIP_6[(P >> 8) & 0x3f] & 0xff;
	n_flipped += COUNT_FLIP_6[((P & 0x0102040810200000ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_H2(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_1[((P & 0x8080808080800000ULL) * 0x0002040810204081ULL) >> 58] & 0xff;
	n_flipped += COUNT_FLIP_7[(P >> 8) & 0x7f] & 0xff;
	n_flipped += COUNT_FLIP_6[((P & 0x0204081020400000ULL) * 0x0101010101010101ULL) >> 57] & 0xff;
...
static int count_last_flip_A3(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_2[((P & 0x2010080402010101ULL) * 0x0102040404040404ULL) >> 56] & 0xff;	// A1A3F8
	n_flipped += COUNT_FLIP_0[(P >> 17) & 0x7f] & 0xff;
	n_flipped += COUNT_FLIP_5[((P & 0x0101010101010204ULL) * 0x2020201008040201ULL) >> 56] & 0xff;	// C1A3A8
...
static int count_last_flip_B3(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_2[((P & 0x4020100804020202ULL) * 0x0081020202020202ULL) >> 56] & 0xff;	// B1B3G8
	n_flipped += COUNT_FLIP_1[(P >> 18) & 0x3f] & 0xff;
	n_flipped += COUNT_FLIP_5[(((P & 0x0202020202020408ULL) >> 1) * 0x2020201008040201ULL) >> 56] & 0xff;	// D1B3B8
...
static int count_last_flip_C3(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_2[((P & 0x0404040404040404ULL) * 0x0040810204081020ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_2[(P >> 16) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_2[((P & 0x0000000102040810ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_2[((P & 0x8040201008040201ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_D3(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_2[((P & 0x0808080808080808ULL) * 0x0020408102040810ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_3[(P >> 16) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_3[((P & 0x0000010204081020ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_3[((P & 0x0080402010080402ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_E3(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_2[((P & 0x1010101010101010ULL) * 0x0010204081020408ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_4[(P >> 16) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_4[((P & 0x0001020408102040ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_4[((P & 0x0000804020100804ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_F3(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_2[((P & 0x2020202020202020ULL) * 0x0008102040810204ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_5[(P >> 16) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_5[((P & 0x0102040810204080ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_5[((P & 0x0000008040201008ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_G3(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_2[((P & 0x4040404040402010ULL) * 0x0010101020408102ULL) >> 56] & 0xff;	// E1G3G8
	n_flipped += COUNT_FLIP_6[(P >> 16) & 0x3f] & 0xff;
	n_flipped += COUNT_FLIP_5[(((P & 0x0204081020404040ULL) >> 1) * 0x0402010101010101ULL) >> 56] & 0xff;	// G1G3B8
...
static int count_last_flip_H3(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_2[((P & 0x8080808080804020ULL) * 0x0008080810204081ULL) >> 56] & 0xff;	// F1H3H8
	n_flipped += COUNT_FLIP_7[(P >> 16) & 0x7f] & 0xff;
	n_flipped += COUNT_FLIP_5[(((P & 0x0408102040808080ULL) >> 2) * 0x0402010101010101ULL) >> 56] & 0xff;	// H1H3C8
...
static int count_last_flip_A4(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_3[((P & 0x1008040201010101ULL) * 0x0102040808080808ULL) >> 56] & 0xff;	// A1A4E8
	n_flipped += COUNT_FLIP_0[(P >> 25) & 0x7f] & 0xff;
	n_flipped += COUNT_FLIP_4[((P & 0x0101010101020408ULL) * 0x1010101008040201ULL) >> 56] & 0xff;	// D1A4A8
...
static int count_last_flip_B4(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_3[((P & 0x2010080402020202ULL) * 0x0081020404040404ULL) >> 56] & 0xff;	// B1B4F8
	n_flipped += COUNT_FLIP_1[(P >> 26) & 0x3f] & 0xff;
	n_flipped += COUNT_FLIP_4[(((P & 0x0202020202040810ULL) >> 1) * 0x1010101008040201ULL) >> 56] & 0xff;	// E1B4B8
...
static int count_last_flip_C4(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_3[((P & 0x0404040404040404ULL) * 0x0040810204081020ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_2[(P >> 24) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_2[((P & 0x0000010204081020ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_2[((P & 0x4020100804020100ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_D4(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_3[((P & 0x0808080808080808ULL) * 0x0020408102040810ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_3[(P >> 24) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_3[((P & 0x0001020408102040ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_3[((P & 0x8040201008040201ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_E4(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_3[((P & 0x1010101010101010ULL) * 0x0010204081020408ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_4[(P >> 24) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_4[((P & 0x0102040810204080ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_4[((P & 0x0080402010080402ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_F4(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_3[((P & 0x2020202020202020ULL) * 0x0008102040810204ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_5[(P >> 24) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_5[((P & 0x0204081020408000ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_5[((P & 0x0000804020100804ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_G4(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_3[((P & 0x4040404040201008ULL) * 0x0020202020408102ULL) >> 56] & 0xff;	// D1G4G8
	n_flipped += COUNT_FLIP_6[(P >> 24) & 0x3f] & 0xff;
	n_flipped += COUNT_FLIP_4[(((P & 0x0408102040404040ULL) >> 2) * 0x0804020101010101ULL) >> 56] & 0xff;	// G1G4C8
...
static int count_last_flip_H4(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_3[((P & 0x8080808080402010ULL) * 0x0010101010204081ULL) >> 56] & 0xff;	// E1H4H8
	n_flipped += COUNT_FLIP_7[(P >> 24) & 0x7f] & 0xff;
	n_flipped += COUNT_FLIP_4[(((P & 0x0810204080808080ULL) >> 3) * 0x0804020101010101ULL) >> 56] & 0xff;	// H1H4D8
...
static int count_last_flip_A5(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_4[((P & 0x0804020101010101ULL) * 0x0102040810101010ULL) >> 56] & 0xff;	// A1A5D8
	n_flipped += COUNT_FLIP_0[(P >> 33) & 0x7f] & 0xff;
	n_flipped += COUNT_FLIP_3[((P & 0x0101010102040810ULL) * 0x0808080808040201ULL) >> 56] & 0xff;	// E1A5A8
...
static int count_last_flip_B5(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_4[((P & 0x1008040202020202ULL) * 0x0081020408080808ULL) >> 56] & 0xff;	// B1B5E8
	n_flipped += COUNT_FLIP_1[(P >> 34) & 0x3f] & 0xff;
	n_flipped += COUNT_FLIP_3[(((P & 0x0202020204081020ULL) >> 1) * 0x0808080808040201ULL) >> 56] & 0xff;	// F1B5B8
...
static int count_last_flip_C5(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_4[((P & 0x0404040404040404ULL) * 0x0040810204081020ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_2[(P >> 32) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_2[((P & 0x0001020408102040ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_2[((P & 0x2010080402010000ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_D5(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_4[((P & 0x0808080808080808ULL) * 0x0020408102040810ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_3[(P >> 32) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_3[((P & 0x0102040810204080ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_3[((P & 0x4020100804020100ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_E5(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_4[((P & 0x1010101010101010ULL) * 0x0010204081020408ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_4[(P >> 32) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_4[((P & 0x0204081020408000ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_4[((P & 0x8040201008040201ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_F5(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_4[((P & 0x2020202020202020ULL) * 0x0008102040810204ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_5[(P >> 32) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_5[((P & 0x0408102040800000ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_5[((P & 0x0080402010080402ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_G5(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_4[((P & 0x4040404020100804ULL) * 0x0040404040408102ULL) >> 56] & 0xff;	// C1G5G8
	n_flipped += COUNT_FLIP_6[(P >> 32) & 0x3f] & 0xff;
	n_flipped += COUNT_FLIP_3[(((P & 0x0810204040404040ULL) >> 3) * 0x1008040201010101ULL) >> 56] & 0xff;	// G1G5D8
...
static int count_last_flip_H5(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_4[((P & 0x8080808040201008ULL) * 0x0020202020204081ULL) >> 56] & 0xff;	// D1H5H8
	n_flipped += COUNT_FLIP_7[(P >> 32) & 0x7f] & 0xff;
	n_flipped += COUNT_FLIP_3[(((P & 0x1020408080808080ULL) >> 4) * 0x1008040201010101ULL) >> 56] & 0xff;	// H1H5E8
...
static int count_last_flip_A6(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_5[((P & 0x0402010101010101ULL) * 0x0102040810202020ULL) >> 56] & 0xff;	// A1A6C8
	n_flipped += COUNT_FLIP_0[(P >> 41) & 0x7f] & 0xff;
	n_flipped += COUNT_FLIP_2[((P & 0x0101010204081020ULL) * 0x0404040404040201ULL) >> 56] & 0xff;	// F1A6A8
...
static int count_last_flip_B6(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_5[((P & 0x0804020202020202ULL) * 0x0081020408101010ULL) >> 56] & 0xff;	// B1B6D8
	n_flipped += COUNT_FLIP_1[(P >> 42) & 0x3f] & 0xff;
	n_flipped += COUNT_FLIP_2[(((P & 0x0202020408102040ULL) >> 1) * 0x0404040404040201ULL) >> 56] & 0xff;	// G1B6B8
...
static int count_last_flip_C6(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_5[((P & 0x0404040404040404ULL) * 0x0040810204081020ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_2[(P >> 40) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_2[((P & 0x0102040810204080ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_2[((P & 0x1008040201000000ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_D6(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_5[((P & 0x0808080808080808ULL) * 0x0020408102040810ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_3[(P >> 40) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_3[((P & 0x0204081020408000ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_3[((P & 0x2010080402010000ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_E6(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_5[((P & 0x1010101010101010ULL) * 0x0010204081020408ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_4[(P >> 40) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_4[((P & 0x0408102040800000ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_4[((P & 0x4020100804020100ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_F6(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_5[((P & 0x2020202020202020ULL) * 0x0008102040810204ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_5[(P >> 40) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_5[((P & 0x0810204080000000ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_5[((P & 0x8040201008040201ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_G6(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_5[((P & 0x4040402010080402ULL) * 0x0080808080808102ULL) >> 56] & 0xff;	// B1G6G8
	n_flipped += COUNT_FLIP_6[(P >> 40) & 0x3f] & 0xff;
	n_flipped += COUNT_FLIP_2[(((P & 0x1020404040404040ULL) >> 4) * 0x2010080402010101ULL) >> 56] & 0xff;	// G1G6E8
...
static int count_last_flip_H6(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_5[((P & 0x8080804020100804ULL) * 0x0040404040404081ULL) >> 56] & 0xff;	// C1H6H8
	n_flipped += COUNT_FLIP_7[(P >> 40) & 0x7f] & 0xff;
	n_flipped += COUNT_FLIP_2[(((P & 0x2040808080808080ULL) >> 5) * 0x2010080402010101ULL) >> 56] & 0xff;	// H1H6F8
...
static int count_last_flip_A7(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_6[((P & 0x0000010101010101ULL) * 0x0102040810204080ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_0[(P >> 49) & 0x7f] & 0xff;
	n_flipped += COUNT_FLIP_1[((P & 0x0000020408102040ULL) * 0x0101010101010101ULL) >> 57] & 0xff;
...
static int count_last_flip_B7(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_6[((P & 0x0000020202020202ULL) * 0x0081020408102040ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_1[(P >> 50) & 0x3f] & 0xff;
	n_flipped += COUNT_FLIP_1[((P & 0x0000040810204080ULL) * 0x0101010101010101ULL) >> 58] & 0xff;
...
static int count_last_flip_C7(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_6[((P & 0x0000040404040404ULL) * 0x0040810204081020ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_2[(P >> 48) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_2[(P & 0x00040a1120408000ULL) * 0x0101010101010101ULL >> 56] & 0xff;	// A5C7H2
...
static int count_last_flip_D7(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_6[((P & 0x0000080808080808ULL) * 0x0020408102040810ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_3[(P >> 48) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_3[(P & 0x0008142241800000ULL) * 0x0101010101010101ULL >> 56] & 0xff;	// A4D7H3
...
static int count_last_flip_E7(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_6[((P & 0x0000101010101010ULL) * 0x0010204081020408ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_4[(P >> 48) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_4[(P & 0x0010284482010000ULL) * 0x0101010101010101ULL >> 56] & 0xff;	// A3E7H4
...
static int count_last_flip_F7(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_6[((P & 0x0000202020202020ULL) * 0x0008102040810204ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_5[(P >> 48) & 0xff] & 0xff;
	n_flipped += COUNT_FLIP_5[(P & 0x0020508804020100ULL) * 0x0101010101010101ULL >> 56] & 0xff;	// A2F7H5
...
static int count_last_flip_G7(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_6[((P & 0x0000404040404040ULL) * 0x0004081020408102ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_6[(P >> 48) & 0x3f] & 0xff;
	n_flipped += COUNT_FLIP_6[((P & 0x0000201008040201ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_H7(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_6[((P & 0x0000808080808080ULL) * 0x0002040810204081ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_7[(P >> 48) & 0x7f] & 0xff;
	n_flipped += COUNT_FLIP_6[((P & 0x0000402010080402ULL) * 0x0101010101010101ULL) >> 57] & 0xff;
...
static int count_last_flip_A8(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_7[((P & 0x0001010101010101ULL) * 0x0102040810204080ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_0[P >> 57] & 0xff;
	n_flipped += COUNT_FLIP_0[((P & 0x0002040810204080ULL) * 0x0101010101010101ULL) >> 57] & 0xff;
...
static int count_last_flip_B8(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_7[((P & 0x0002020202020202ULL) * 0x0081020408102040ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_1[P >> 58] & 0xff;
	n_flipped += COUNT_FLIP_1[((P & 0x0004081020408000ULL) * 0x0101010101010101ULL) >> 58] & 0xff;
...
static int count_last_flip_C8(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_7[((P & 0x0004040404040404ULL) * 0x0040810204081020ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_2[P >> 56] & 0xff;
	n_flipped += COUNT_FLIP_2[(P & 0x040a112040800000ULL) * 0x0101010101010101ULL >> 56] & 0xff;	// A6C8H3
...
static int count_last_flip_D8(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_7[((P & 0x0008080808080808ULL) * 0x0020408102040810ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_3[P >> 56] & 0xff;
	n_flipped += COUNT_FLIP_3[(P & 0x0814224180000000ULL) * 0x0101010101010101ULL >> 56] & 0xff;	// A5D8H4
...
static int count_last_flip_E8(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_7[((P & 0x0010101010101010ULL) * 0x0010204081020408ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_4[P >> 56] & 0xff;
	n_flipped += COUNT_FLIP_4[(P & 0x1028448201000000ULL) * 0x0101010101010101ULL >> 56] & 0xff;	// A4E8H5
...
static int count_last_flip_F8(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_7[((P & 0x0020202020202020ULL) * 0x0008102040810204ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_5[P >> 56] & 0xff;
	n_flipped += COUNT_FLIP_5[(P & 0x2050880402010000ULL) * 0x0101010101010101ULL >> 56] & 0xff;	// A3F8H6
...
static int count_last_flip_G8(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_7[((P & 0x0040404040404040ULL) * 0x0004081020408102ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_6[(P >> 56) & 0x3f] & 0xff;
	n_flipped += COUNT_FLIP_6[((P & 0x0020100804020100ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
...
static int count_last_flip_H8(const unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP_7[((P & 0x0080808080808080ULL) * 0x0002040810204081ULL) >> 56] & 0xff;
	n_flipped += COUNT_FLIP_7[(P >> 56) & 0x7f] & 0xff;
	n_flipped += COUNT_FLIP_7[((P & 0x0040201008040201ULL) * 0x0101010101010101ULL) >> 56] & 0xff;
